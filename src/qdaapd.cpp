...
QDAAPd::QDAAPd( QDaap::Collection * c, quint16 port) :
...
void QDAAPd::handleRequest(struct mg_connection * conn, const struct mg_request_info * request_info)
{
...
QByteArray QDAAPd::cs(const TagOutput &resp)
{
    Chunk chunk = resp.data();
    return QByteArray((const char*)chunk.begin(), (int)chunk.size());
}

int QDAAPd::do_server_info(QStringList & headers, QByteArray & body)
{
    TagOutput resp;
    resp    << Tag('msrv')
...
int QDAAPd::do_login(QStringList & headers, QByteArray & body)
{
    TagOutput resp;
    resp    << Tag('mlog')
                << Tag('mstt') << (u32)200 << end
...
int QDAAPd::do_databases(QStringList & headers, QByteArray & body)
{
    TagOutput resp;
    resp    << Tag('avdb')
                << Tag('mstt') << (u32)200 << end
...
int QDAAPd::do_databases_items(QStringList & headers, QByteArray & body)
{
    TagOutput resp;
    resp    << Tag('adbs')
                << Tag('mstt') << (u32)200 << end
...
int QDAAPd::do_databases_containers(QStringList & headers, QByteArray & body)
{
    TagOutput resp;
    resp    << Tag('apso')
                << Tag('mstt') << (u32)200 << end
...
//TODO playlists.
int QDAAPd::do_databases_containers_items(QStringList & headers, QByteArray & body)
{
...
int QDAAPd::do_update(QStringList & headers, QByteArray & body)
{
